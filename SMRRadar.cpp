...
CSMRRadar::CSMRRadar()
{
	...
	std::string dir(path);
	std::string filename("vSMR.dll");
	size_t pos = dir.find(filename);
	dir.replace(pos, filename.length(), "");
	dir += "airports.csv";
	io::CSVReader<5, io::trim_chars<' '>, io::double_quote_escape<';', '\"'>> inAirport(dir);
	...
	Logger::info("Airport regexes read successfully.");
	...
	if (ObjectType == TAG_CITEM_CALLSIGN || ObjectType == TAG_CITEM_FPBOX || ObjectType == TAG_CITEM_RWY || ObjectType == TAG_CITEM_SID || ObjectType == TAG_CITEM_GATE || ObjectType == TAG_CITEM_NO || ObjectType == TAG_CITEM_GROUNDSTATUS) {
		...
		case TAG_CITEM_CALLSIGN:
			if (Button == EuroScopePlugIn::BUTTON_LEFT || Button == EuroScopePlugIn::BUTTON_MIDDLE)
				StartTagFunction(rt.GetCallsign(), NULL, TAG_CITEM_CALLSIGN, rt.GetCallsign(), NULL, EuroScopePlugIn::TAG_ITEM_FUNCTION_SET_GROUND_STATUS, Pt, Area);
...
map<string, string> CSMRRadar::GenerateTagData(CRadarTarget rt, CFlightPlan fp, bool isAcCorrelated, bool isProMode, int TransitionAltitude, bool useSpeedForGates)
{
	...
	else
	{
			...
			string remarks = fpdata.GetRemarks();
			auto dest = fpdata.GetDestination();
			auto foundDest = std::find(activeAirports.begin(), activeAirports.end(), dest);
			if (foundDest != activeAirports.end())
			{
				for (auto re : foundDest->m_standregex)
				{
					smatch match;
